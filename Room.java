...
public class Room
{
    ...
    public Room(int rNum)
    {
     ...
     switch(roomIndex)
     {
      ...
      objects[0].setLoc(6*40,2*40); //bottom door
      objects[1].setLoc(3*40,6*40); //right door
      break;
      ...
      objects[0].setLoc(2*40,0); //left door
      objects[1].setLoc(2*40,9*40); //right door
      break;
      
      case 4: //Room 
      dimensions = new Rectangle(SIZE4);
      objects = new Obstacle[]{new Doorway(3), new Doorway(5)};
      objects[0].setLoc(3*40,0); //left door
      objects[1].setLoc(0,3*40); //top door
      break;
      
      case 5: //Room 5
      dimensions = new Rectangle(SIZE3);
      objects = new Obstacle[]{new Doorway(4), new Doorway(6)};
      objects[0].setLoc(9*40,7*40); //bottom door
      objects[1].setLoc(5*40,0); //left door
      break;
      
      case 6: //Room 6
      dimensions = new Rectangle(SIZE2);
      objects = new Obstacle[]{new Doorway(5), new Doorway(7)};
      objects[0].setLoc(2*40,14*40); //right door
      objects[1].setLoc(0,2*40); //top door
      break;
      
      case 7: //Room 7
      dimensions = new Rectangle(SIZE1);
      objects = new Obstacle[]{new Doorway(6), new Doorway(8)};
      objects[0].setLoc(2*40,0); //bottom door
      objects[1].setLoc(2*40,9*40); //right door
      break;
